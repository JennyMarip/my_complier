...
{
       0,    56,    56,    67,    68,    70,    71,    72,    73,    74,
      76,    77,    80,    81,    83,    84,    85,    87,    88,    90,
      93,    94,    95,    97,    98,    99,   100,   102,   103,   105,
     108,   109,   111,   112,   114,   115,   116,   117,   118,   119,
     120,   121,   122,   123,   124,   127,   128,   130,   131,   133,
     134,   136,   137,   140,   141,   142,   143,   144,   145,   146,
...
#line 56 "syntax.y"
			...
			if(has_error==0){
			    ...
			    add_read_write();
			    semantic((yyval.type_node));
			    gen_mid_code((yyval.type_node));
			}
			}
...
void preorder_tra(struct node* root,int depth,FILE* fp){
    ...
    if(strcmp(root->id,"null")!=0){
    	...
    	fputs(root->id, fp);
    	if(strcmp(root->id,"TYPE")==0||strcmp(root->id,"ID")==0||strcmp(root->id,"INT")==0||strcmp(root->id,"FLOAT")==0){
    	    fputs(": ", fp);
    	    fputs(root->value, fp);
    	}
    	else {
